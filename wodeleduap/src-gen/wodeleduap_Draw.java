...
public class wodeleduap_Draw implements manager.IMutatorDraw {
			...
			if (source.length() > 0 && target.length() > 0) {
				...
				for (HashMap<String, String> rel : rels) {
					for (String key : rel.keySet()) {
						...
						System.out.println(rel.get(key));
						for (String keyParameters : parameters.keySet()) {
							System.out.println(keyParameters);
							System.out.println(parameters.get(keyParameters));
	...
	public static void generateGraphs(File file, String folder, List<EPackage> packages, File exercise)
		...
		if (file.isFile()) {
			...
			if (pathfile.endsWith(".model") == true) {
				...
				String dotfile = "C:/eclipse/runtime-Wodel-Edu-AP/null/src-gen/html/diagrams/" + "/" + path
						+ file.getName().replace(".model", ".dot");
				String pngfile = "C:/eclipse/runtime-Wodel-Edu-AP/null/src-gen/html/diagrams/" + "/" + path
				...
				dotcode.add("}");
				File diagramsfolder = new File("C:/eclipse/runtime-Wodel-Edu-AP/null/src-gen/html/diagrams/");
				if (diagramsfolder.exists() != true) {
					diagramsfolder.mkdir();
				}
				File exercisefolder = new File(
						"C:/eclipse/runtime-Wodel-Edu-AP/null/src-gen/html/diagrams/" + exercise.getName() + "/");
				...
				if (folders.size() > 0) {
					for (String folderName : folders) {
						path += folderName + "/";
						exercisefolder = new File("C:/eclipse/runtime-Wodel-Edu-AP/null/src-gen/html/diagrams/" + path);
						if (exercisefolder.exists() != true) {
							exercisefolder.mkdir();
						}
					}
	...
	public void generate() throws MetaModelNotFoundException, ModelNotFoundException, FileNotFoundException {
		...
		File folder = new File("C:/eclipse/runtime-Wodel-Edu-AP/null/data/model");
		for (File file : folder.listFiles()) {
			if (file.isFile()) {
				String pathfile = file.getPath();
				if (pathfile.endsWith(".model") == true) {
					Resource model = ModelManager.loadModel(packages, pathfile);
					String dotfile = "C:/eclipse/runtime-Wodel-Edu-AP/null/src-gen/html/diagrams/"
							+ file.getName().replace(".model", "") + "/" + file.getName().replace(".model", ".dot");
					String pngfile = "C:/eclipse/runtime-Wodel-Edu-AP/null/src-gen/html/diagrams/"
					...
					File dotfolder = new File("C:/eclipse/runtime-Wodel-Edu-AP/null/src-gen/html/diagrams/"
		...
		folder = new File("C:/eclipse/runtime-Wodel-Edu-AP/null/data/out");
					...
					if (file.isFile()) {
						...
						if (pathfile.endsWith(".model") == true) {
							...
							File diagramsfolder = new File(
									"C:/eclipse/runtime-Wodel-Edu-AP/null/src-gen/html/diagrams/");
							if (diagramsfolder.exists() != true) {
								diagramsfolder.mkdir();
							}
							File dotfolder = new File("C:/eclipse/runtime-Wodel-Edu-AP/null/src-gen/html/diagrams/"
