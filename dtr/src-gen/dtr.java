...
public class dtr extends manager.MutatorUtils implements manager.IMutatorExecutor {
	...
	public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics,
		...
		for (String modelFilename : modelFilenames) {
			...
			Resource mutatormodel = ModelManager.loadModel(mutatorpackages,
					URI.createURI("file:/C:/eclipse/workspace/dtr/data/out/dtr.model").toFileString());
			...
			for (int i = 0; i < numMutants; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> l66 = mutation1(packages, model, hashmapEObject, hashmapList);
						if (l66 != null) {
							int k = 0;
							for (Mutator mut : l66) {
								if (mut != null) {
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/dtr/data/out/",
					"C:/eclipse/workspace/dtr/data/model/DFAAutomaton.ecore", "C:/eclipse/workspace/dtr/data/model/",
					"dtr.java", hashmapMutVersions);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/dtr/data/out/",
					"C:/eclipse/workspace/dtr/data/model/DFAAutomaton.ecore", "C:/eclipse/workspace/dtr/data/model/",
					"dtr.java", hashmapMutVersions);
			metricsGenerator.run();
			monitor.worked(1);
