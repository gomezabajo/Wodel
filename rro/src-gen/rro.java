...
public class rro extends manager.MutatorUtils implements manager.IMutatorExecutor {
	private ArrayList<Mutator> mutation1(List<EPackage> packages, Resource model, HashMap<String, EObject> hmObjects,
		...
		if (objectSelection != null) {
			...
			atts.add(new ListStringConfigurationStrategy(
					(String) ModelManager.getAttribute("operator",
							(objectSelection != null) ? objectSelection.getObject() : null),
					"[less, equals, greater, distinct]", "operator"));
	...
	public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics,
		...
		String ecoreURI = "C:/eclipse/workspace/rro/data/model/Asple.ecore";
		String modelURI = "C:/eclipse/workspace/rro/data/model/";
		String modelsURI = "C:/eclipse/workspace/rro/data/out/";
		...
		for (String modelFilename : modelFilenames) {
			...
			Resource mutatormodel = ModelManager.loadModel(mutatorpackages,
					URI.createURI("file:/C:/eclipse/workspace/rro/data/out/rro.model").toFileString());
			...
			for (int i = 0; i < numMutants; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> l21 = mutation1(packages, model, hashmapEObject, hashmapList);
						if (l21 != null) {
							int k = 0;
							for (Mutator mut : l21) {
								if (mut != null) {
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/rro/data/out/",
					"C:/eclipse/workspace/rro/data/model/Asple.ecore", "C:/eclipse/workspace/rro/data/model/",
					"rro.java", hashmapMutVersions);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/rro/data/out/",
					"C:/eclipse/workspace/rro/data/model/Asple.ecore", "C:/eclipse/workspace/rro/data/model/",
					"rro.java", hashmapMutVersions);
			metricsGenerator.run();
			monitor.worked(1);
