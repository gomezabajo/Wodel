...
public class isv extends manager.MutatorUtils implements manager.IMutatorExecutor {
	...
	private ArrayList<Mutator> mutation2(List<EPackage> packages, Resource model, HashMap<String, EObject> hmObjects,
		...
		((ReferenceEvaluation) exp0.first).refName = null;
		((ReferenceEvaluation) exp0.first).operator = "different";
		((ReferenceEvaluation) exp0.first).value = new SpecificObjectSelection(packages, model, hmObjects.get("v1"))
				.getObject();
		exp0.operator = new ArrayList<Operator>();
		...
		AttributeEvaluation ev1_0 = new AttributeEvaluation();
		ev1_0.name = "name";
	...
	private ArrayList<Mutator> mutation3(List<EPackage> packages, Resource model, HashMap<String, EObject> hmObjects,
		...
		if (objectSelection != null && objectSelection.getObject() != null) {
			if (hmObjects.get("v2") != null) {
				...
				atts.add(new CopyAttributeConfigurationStrategy(objectSelection.getObject(), hmObjects.get("v2"),
						"name", "name"));
	...
	public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics,
		...
		String ecoreURI = "C:/eclipse/workspace/isv/data/model/bpel.ecore";
		String modelURI = "C:/eclipse/workspace/isv/data/model/";
		String modelsURI = "C:/eclipse/workspace/isv/data/out/";
		...
		for (String modelFilename : modelFilenames) {
			...
			Resource mutatormodel = ModelManager.loadModel(mutatorpackages,
					URI.createURI("file:/C:/eclipse/workspace/isv/data/out/isv.model").toFileString());
			...
			for (int i = 0; i < numMutants; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> lv11 = mutation1(packages, model, hashmapEObject, hashmapList);
						if (lv11 != null) {
							int k = 0;
							for (Mutator mut : lv11) {
								if (mut != null) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> lv22 = mutation2(packages, model, hashmapEObject, hashmapList);
						if (lv22 != null) {
							int k = 0;
							for (Mutator mut : lv22) {
								if (mut != null) {
									Object mutated = mut.mutate();
									if (mutated != null) {
										if (mutated instanceof EObject) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> l3 = mutation3(packages, model, hashmapEObject, hashmapList);
						if (l3 != null) {
							int k = 0;
							for (Mutator mut : l3) {
								if (mut != null) {
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/isv/data/out/",
					"C:/eclipse/workspace/isv/data/model/bpel.ecore", "C:/eclipse/workspace/isv/data/model/",
					"isv.java", hashmapMutVersions);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/isv/data/out/",
					"C:/eclipse/workspace/isv/data/model/bpel.ecore", "C:/eclipse/workspace/isv/data/model/",
					"isv.java", hashmapMutVersions);
			metricsGenerator.run();
			monitor.worked(1);
