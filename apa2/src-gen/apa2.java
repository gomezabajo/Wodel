...
public class apa2 extends manager.MutatorUtils implements manager.IMutatorExecutor {
	...
	private ArrayList<Mutator> mutation2(List<EPackage> packages, Resource model, HashMap<String, EObject> hmObjects,
		...
		RandomTypeSelection rts = new RandomTypeSelection(packages, model, "OnAlarm", referenceSelection,
				containerSelection);
	...
	public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics,
		...
		String ecoreURI = "C:/eclipse/workspace/apa2/data/model/bpel.ecore";
		String modelURI = "C:/eclipse/workspace/apa2/data/model/";
		String modelsURI = "C:/eclipse/workspace/apa2/data/out/";
		...
		for (String modelFilename : modelFilenames) {
			...
			Resource mutatormodel = ModelManager.loadModel(mutatorpackages,
					URI.createURI("file:/C:/eclipse/workspace/apa2/data/out/apa2.model").toFileString());
			...
			for (int i = 0; i < numMutants; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> lev110 = mutation1(packages, model, hashmapEObject, hashmapList);
						if (lev110 != null) {
							int k = 0;
							for (Mutator mut : lev110) {
								if (mut != null) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> la311 = mutation2(packages, model, hashmapEObject, hashmapList);
						if (la311 != null) {
							int k = 0;
							for (Mutator mut : la311) {
								if (mut != null) {
									Object mutated = mut.mutate();
									if (mutated != null) {
										if (mutated instanceof EObject) {
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/apa2/data/out/",
					"C:/eclipse/workspace/apa2/data/model/bpel.ecore", "C:/eclipse/workspace/apa2/data/model/",
					"apa2.java", hashmapMutVersions);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/apa2/data/out/",
					"C:/eclipse/workspace/apa2/data/model/bpel.ecore", "C:/eclipse/workspace/apa2/data/model/",
					"apa2.java", hashmapMutVersions);
			metricsGenerator.run();
			monitor.worked(1);
