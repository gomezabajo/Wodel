...
public class uat1 extends manager.MutatorUtils implements manager.IMutatorExecutor {
	private ArrayList<Mutator> mutation1(List<EPackage> packages, Resource model, HashMap<String, EObject> hmObjects,
		...
		if (objectSelection != null && objectSelection.getObject() != null) {
			mut = new CloneObjectMutator(model, packages, objectSelection.getObject(), false, referenceSelection,
					containerSelection, atts, refs, "Attribute");
		}
		if (mut != null) {
			mut.setId("m1");
			mutations.add(mut);
	...
	public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics,
		...
		String ecoreURI = "C:/eclipse/workspace/uat1/data/model/UMLClassDiagram.ecore";
		String modelURI = "C:/eclipse/workspace/uat1/data/model/";
		String modelsURI = "C:/eclipse/workspace/uat1/data/out/";
		...
		for (String modelFilename : modelFilenames) {
			...
			Resource mutatormodel = ModelManager.loadModel(mutatorpackages,
					URI.createURI("file:/C:/eclipse/workspace/uat1/data/out/uat1.model").toFileString());
			...
			for (int i = 0; i < numMutants; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> l286 = mutation1(packages, model, hashmapEObject, hashmapList);
						if (l286 != null) {
							int k = 0;
							for (Mutator mut : l286) {
								if (mut != null) {
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/uat1/data/out/",
					"C:/eclipse/workspace/uat1/data/model/UMLClassDiagram.ecore",
					"C:/eclipse/workspace/uat1/data/model/", "uat1.java", hashmapMutVersions);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/uat1/data/out/",
					"C:/eclipse/workspace/uat1/data/model/UMLClassDiagram.ecore",
					"C:/eclipse/workspace/uat1/data/model/", "uat1.java", hashmapMutVersions);
			metricsGenerator.run();
			monitor.worked(1);
