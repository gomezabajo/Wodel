...
public class ccs extends manager.MutatorUtils implements manager.IMutatorExecutor {
	...
	private ArrayList<Mutator> mutation2(List<EPackage> packages, Resource model, HashMap<String, EObject> hmObjects,
		...
		if (hmObjects.get("s") != null) {
			refSelection17 = new SpecificObjectSelection(packages, model, hmObjects.get("s"));
		} else {
			if (hmList.get("s") != null) {
				refSelection17 = new SpecificObjectSelection(packages, model, hmList.get("s"));
			} else {
				return mutations;
			}
		}
		refs.put("tar", refSelection17);
		RandomTypeSelection refRts18 = new RandomTypeSelection(packages, model, "Symbol");
		EObject refObject18 = refRts18.getObject();
		ObSelectionStrategy refSelection18 = null;
		if (refObject18 != null) {
			refSelection18 = new SpecificObjectSelection(packages, model, refObject18);
		}
		refs.put("symbol", refSelection18);
		CreateObjectMutator mut = new CreateObjectMutator(model, packages, referenceSelection, containerSelection, atts,
				refs, "Transition");
		if (mut != null) {
			mut.setId("m2");
	...
	public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics,
		...
		for (String modelFilename : modelFilenames) {
			...
			Resource mutatormodel = ModelManager.loadModel(mutatorpackages,
					URI.createURI("file:/C:/eclipse/workspace/ccs/data/out/ccs.model").toFileString());
			...
			for (int i = 0; i < numMutants; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> ls39 = mutation1(packages, model, hashmapEObject, hashmapList);
						if (ls39 != null) {
							int k = 0;
							for (Mutator mut : ls39) {
								if (mut != null) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> lt40 = mutation2(packages, model, hashmapEObject, hashmapList);
						if (lt40 != null) {
							int k = 0;
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/ccs/data/out/",
					"C:/eclipse/workspace/ccs/data/model/DFAAutomaton.ecore", "C:/eclipse/workspace/ccs/data/model/",
					"ccs.java", hashmapMutVersions);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/ccs/data/out/",
					"C:/eclipse/workspace/ccs/data/model/DFAAutomaton.ecore", "C:/eclipse/workspace/ccs/data/model/",
					"ccs.java", hashmapMutVersions);
			metricsGenerator.run();
			monitor.worked(1);
