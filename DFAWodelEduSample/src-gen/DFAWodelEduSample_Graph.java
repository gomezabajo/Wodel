...
public class DFAWodelEduSample_Graph implements manager.IMutatorGraph {
	...
	public static void generateGraphs(File file, String folder, ArrayList<EPackage> packages, File exercise) throws ModelNotFoundException, FileNotFoundException {
		if (file.isFile()) {
			...
			if (pathfile.endsWith(".model") == true) {
				...
				String dotfile = "C:/eclipse/runtime-EclipseApplication/DFAWodelEduSample/src-gen/html/diagrams/" + "/" +
					path + file.getName().replace(".model", ".dot");
				String pngfile = "C:/eclipse/runtime-EclipseApplication/DFAWodelEduSample/src-gen/html/diagrams/" + "/" +
...
// COUNTER: 3
			...
			File diagramsfolder = new File("C:/eclipse/runtime-EclipseApplication/DFAWodelEduSample/src-gen/html/diagrams/");
			if (diagramsfolder.exists() != true) {
				diagramsfolder.mkdir();
			}
			File exercisefolder = new File("C:/eclipse/runtime-EclipseApplication/DFAWodelEduSample/src-gen/html/diagrams/" + exercise.getName() + "/");
			...
			if (folders.size() > 0) {
				for (String folderName : folders) {
					path += folderName + "/";
					exercisefolder = new File("C:/eclipse/runtime-EclipseApplication/DFAWodelEduSample/src-gen/html/diagrams/" + path);
					if (exercisefolder.exists() != true) {
						exercisefolder.mkdir();
					}
				}
...
public void generate() throws MetaModelNotFoundException, ModelNotFoundException, FileNotFoundException {
	...
	String metamodel = "C:/eclipse/runtime-EclipseApplication/DFAWodelEduSample/model/DFAAutomaton.ecore";
	ArrayList<EPackage> packages = ModelManager.loadMetaModel(metamodel);

	// GENERATES PNG FILES FROM SOURCE MODELS
	File folder = new File("C:/eclipse/runtime-EclipseApplication/DFAWodelEduSample/model");
	for (File file : folder.listFiles()) {
		if (file.isFile()) {
			...
			if (pathfile.endsWith(".model") == true) {
				...
				File dotfolder = new File("C:/eclipse/runtime-EclipseApplication/DFAWodelEduSample/src-gen/html/diagrams/" + 
					file.getName().replace(".model", "") + "/");
	...
	folder = new File("C:/eclipse/runtime-EclipseApplication/DFAWodelEduSample/out");
		...
		if (exercise.isDirectory()) {
			for (File file : exercise.listFiles()) {
				if (file.isFile()) {
					...
					if (pathfile.endsWith(".model") == true) {
						Resource model = ModelManager.loadModel(packages, pathfile);
						String dotfile = "C:/eclipse/runtime-EclipseApplication/DFAWodelEduSample/src-gen/html/diagrams/" + exercise.getName() + "/" +
							file.getName().replace(".model", ".dot");
						String pngfile = "C:/eclipse/runtime-EclipseApplication/DFAWodelEduSample/src-gen/html/diagrams/" + exercise.getName() + "/" +
							file.getName().replace(".model", ".png");
						HashMap<String, String> dotnodes = new HashMap<String, String>();
						HashMap<String, String> dotedges = new HashMap<String, String>();
						// COUNTER: 0
...
if (value == true) {
		dotnodes.put(name, "shape = doublecircle");
						...
						File dotfolder = new File("C:/eclipse/runtime-EclipseApplication/DFAWodelEduSample/src-gen/html/diagrams/" + exercise.getName() + "/");
						if (dotfolder.exists() != true) {
							dotfolder.mkdir();
						}
						PrintWriter dotwriter = new PrintWriter(dotfile);
