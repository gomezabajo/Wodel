package mutator.testATLMRStandalone;
...
public class testATLMRStandalone extends MutatorUtils {
	...
	public int block_cmr(int maxAttempts, int numMutants, boolean registry, List<EPackage> packages,
		...
		String modelURI = testATLMRStandalone.class.getProtectionDomain().getCodeSource().getLocation().getPath()
				.replace("/bin/", "/") + "data/model/";
		String modelsURI = testATLMRStandalone.class.getProtectionDomain().getCodeSource().getLocation().getPath()
		...
		for (String modelFilename : modelFilenames) {
			...
			numMutants = 2;
			String xmiFilename = testATLMRStandalone.class.getProtectionDomain().getCodeSource().getLocation().getPath()
			...
			for (int i = 0; i < numMutantsToGenerate; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						List<Mutator> l73 = mutation1(packages, model, hashmapEObject, hashmapList, serialize, test,
								classes);
						if (l73 != null) {
							for (Mutator mut : l73) {
								if (mut != null) {
					...
					isRepeated = registryMutantWithBlocksStandalone(ecoreURI, packages, registeredPackages,
							...
							testATLMRStandalone.class, true, false);
					if (isRepeated == false) {
						numMutantsGenerated++;
						k[0] = k[0] + 1;
					}
		...
		for (String modelFilename : modelFilenames) {
			...
			for (int i = 0; i < numMutantsToGenerate; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						List<Mutator> l74 = mutation2(packages, model, hashmapEObject, hashmapList, serialize, test,
								classes);
						if (l74 != null) {
							for (Mutator mut : l74) {
								if (mut != null) {
		...
		for (String modelFilename : modelFilenames) {
			...
			for (int i = 0; i < numMutantsToGenerate; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						List<Mutator> l75 = mutation3(packages, model, hashmapEObject, hashmapList, serialize, test,
								classes);
						if (l75 != null) {
							for (Mutator mut : l75) {
								if (mut != null) {
	...
	public MutationResults execute(int maxAttempts, int numMutants, boolean registry, boolean metrics,
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, testATLMRStandalone.class
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, testATLMRStandalone.class
					.getProtectionDomain().getCodeSource().getLocation().getPath().replace("/bin/", "/") + "data/out/",
					"/testATL/data/model/ATL.ecore",
					testATLMRStandalone.class.getProtectionDomain().getCodeSource().getLocation().getPath()
							.replace("/bin/", "/") + "testATL/data/model/",
