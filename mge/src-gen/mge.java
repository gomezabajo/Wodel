...
public class mge extends manager.MutatorUtils implements manager.IMutatorExecutor {
	...
	public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics,
		...
		String ecoreURI = "C:/eclipse/workspace/mge/data/model/UMLClassDiagram.ecore";
		String modelURI = "C:/eclipse/workspace/mge/data/model/";
		String modelsURI = "C:/eclipse/workspace/mge/data/out/";
		...
		for (String modelFilename : modelFilenames) {
			...
			Resource mutatormodel = ModelManager.loadModel(mutatorpackages,
					URI.createURI("file:/C:/eclipse/workspace/mge/data/out/mge.model").toFileString());
			...
			for (int i = 0; i < numMutants; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> lcl271 = mutation1(packages, model, hashmapEObject, hashmapList);
						if (lcl271 != null) {
							int k = 0;
							for (Mutator mut : lcl271) {
								if (mut != null) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> l272 = mutation2(packages, model, hashmapEObject, hashmapList);
						if (l272 != null) {
							int k = 0;
							for (Mutator mut : l272) {
								if (mut != null) {
									Object mutated = mut.mutate();
									if (mutated != null) {
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/mge/data/out/",
					"C:/eclipse/workspace/mge/data/model/UMLClassDiagram.ecore", "C:/eclipse/workspace/mge/data/model/",
					"mge.java", hashmapMutVersions);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/mge/data/out/",
					"C:/eclipse/workspace/mge/data/model/UMLClassDiagram.ecore", "C:/eclipse/workspace/mge/data/model/",
					"mge.java", hashmapMutVersions);
			metricsGenerator.run();
			monitor.worked(1);
