package mutator.testJavaCOStandalone;
...
public class testJavaCOStandalone extends MutatorUtils {
	...
	private int mutation1(List<EPackage> packages, Resource model,
		...
		for (EObject object : objects) {
			String modelsFolder = ModelManager.getModelsFolder(testJavaCOStandalone.class);
			...
			if (object != null) {
				...
				if (mutator != null) {
					...
					String mutFilename = hashmapModelFilenames.get(modelFilename) + "/cora/Output" + k[0] + ".model";
					boolean isRepeated = registryMutantWithBlocksStandalone(ecoreURI, packages, registeredPackages,
							...
							registry, hashsetMutantsBlock, hashmapModelFilenames, hashmapModelFolders, "cora",
							fromNames, k, mutPaths, hashmapMutVersions, "testJava", serialize, test, classes,
							testJavaCOStandalone.class, true, false);
					if (isRepeated == false) {
						numMutantsGenerated++;
						monitor.worked(1);
						k[0] = k[0] + 1;
	...
	public int block_cora(int maxAttempts, int numMutants, boolean registry, List<EPackage> packages,
		...
		String modelURI = testJavaCOStandalone.class.getProtectionDomain().getCodeSource().getLocation().getPath()
				.replace("/bin/", "/") + "data/model/";
		String modelsURI = testJavaCOStandalone.class.getProtectionDomain().getCodeSource().getLocation().getPath()
		...
		for (String modelFilename : modelFilenames) {
			...
			numMutants = -1;
			String xmiFilename = testJavaCOStandalone.class.getProtectionDomain().getCodeSource().getLocation()
					.getPath().replace("/bin/", "/") + "data/out/testJavaCO.model";
	...
	private int mutation2(List<EPackage> packages, Resource model,
		...
		for (EObject object : objects) {
			...
			if (object != null) {
				...
				if (mutator != null) {
					Map<String, List<String>> rules = new LinkedHashMap<String, List<String>>();
					String mutFilename = hashmapModelFilenames.get(modelFilename) + "/coro/Output" + k[0] + ".model";
					boolean isRepeated = registryMutantWithBlocksStandalone(ecoreURI, packages, registeredPackages,
							localRegisteredPackages, seed, mutator.getModel(), rules, muts, modelFilename, mutFilename,
							registry, hashsetMutantsBlock, hashmapModelFilenames, hashmapModelFolders, "coro",
							fromNames, k, mutPaths, hashmapMutVersions, "testJava", serialize, test, classes,
							testJavaCOStandalone.class, true, false);
					if (isRepeated == false) {
						numMutantsGenerated++;
	...
	public int block_coro(int maxAttempts, int numMutants, boolean registry, List<EPackage> packages,
			Map<String, EPackage> registeredPackages, Map<String, EPackage> localRegisteredPackages,
			List<String> fromNames, Map<String, Set<String>> hashmapMutants,
			Map<String, List<String>> hashmapMutVersions, IProgressMonitor monitor, int[] k, boolean serialize,
			IWodelTest test, Map<String, List<String>> classes)
	...
	private int mutation3(List<EPackage> packages, Resource model,
		...
		for (EObject object : objects) {
			...
			if (object != null) {
				...
				if (mutator != null) {
					Map<String, List<String>> rules = new LinkedHashMap<String, List<String>>();
					String mutFilename = hashmapModelFilenames.get(modelFilename) + "/corx/Output" + k[0] + ".model";
					boolean isRepeated = registryMutantWithBlocksStandalone(ecoreURI, packages, registeredPackages,
							localRegisteredPackages, seed, mutator.getModel(), rules, muts, modelFilename, mutFilename,
							registry, hashsetMutantsBlock, hashmapModelFilenames, hashmapModelFolders, "corx",
							fromNames, k, mutPaths, hashmapMutVersions, "testJava", serialize, test, classes,
							testJavaCOStandalone.class, true, false);
					if (isRepeated == false) {
						numMutantsGenerated++;
	...
	public int block_corx(int maxAttempts, int numMutants, boolean registry, List<EPackage> packages,
			Map<String, EPackage> registeredPackages, Map<String, EPackage> localRegisteredPackages,
			List<String> fromNames, Map<String, Set<String>> hashmapMutants,
			Map<String, List<String>> hashmapMutVersions, IProgressMonitor monitor, int[] k, boolean serialize,
			IWodelTest test, Map<String, List<String>> classes)
	...
	public MutationResults execute(int maxAttempts, int numMutants, boolean registry, boolean metrics,
		...
		if (blockNames == null || (blockNames != null && Arrays.asList(blockNames).contains("cora") == true)) {
			monitor.subTask("Generating mutants for block cora (1/3)");
			int[] k = new int[1];
			k[0] = 0;
			int numMutantsGenerated = block_cora(maxAttempts, numMutants, registry, packages, registeredPackages,
			...
			if (numMutantsGenerated > 0) {
				...
				mutationResults.mutatorsApplied.add("cora");
				mutationResults.numMutantsGenerated += numMutantsGenerated;
			}
			monitor.worked(1);
		}
		fromNames = new ArrayList<String>();
		if (blockNames == null || (blockNames != null && Arrays.asList(blockNames).contains("coro") == true)) {
			monitor.subTask("Generating mutants for block coro (2/3)");
			int[] k = new int[1];
			k[0] = 0;
			int numMutantsGenerated = block_coro(maxAttempts, numMutants, registry, packages, registeredPackages,
					localRegisteredPackages, fromNames, hashmapMutants, hashmapMutVersions, monitor, k, serialize, test,
					classes);
			if (numMutantsGenerated > 0) {
				mutationResults.numMutatorsApplied++;
				...
				mutationResults.mutatorsApplied.add("coro");
				mutationResults.numMutantsGenerated += numMutantsGenerated;
			}
			monitor.worked(1);
		}
		fromNames = new ArrayList<String>();
		if (blockNames == null || (blockNames != null && Arrays.asList(blockNames).contains("corx") == true)) {
			monitor.subTask("Generating mutants for block corx (3/3)");
			int[] k = new int[1];
			k[0] = 0;
			int numMutantsGenerated = block_corx(maxAttempts, numMutants, registry, packages, registeredPackages,
					localRegisteredPackages, fromNames, hashmapMutants, hashmapMutVersions, monitor, k, serialize, test,
					classes);
			if (numMutantsGenerated > 0) {
				mutationResults.numMutatorsApplied++;
				...
				mutationResults.mutatorsApplied.add("corx");
				mutationResults.numMutantsGenerated += numMutantsGenerated;
			}
			monitor.worked(1);
		}
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, testJavaCOStandalone.class
					.getProtectionDomain().getCodeSource().getLocation().getPath().replace("/bin/", "/") + "data/out/",
					"/testJava/data/model/java.ecore",
					testJavaCOStandalone.class.getProtectionDomain().getCodeSource().getLocation().getPath()
							.replace("/bin/", "/") + "testJava/data/model/",
					"testJavaCOStandalone.java", hashmapMutVersions, testJavaCOStandalone.class);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, testJavaCOStandalone.class
					.getProtectionDomain().getCodeSource().getLocation().getPath().replace("/bin/", "/") + "data/out/",
					"/testJava/data/model/java.ecore",
					testJavaCOStandalone.class.getProtectionDomain().getCodeSource().getLocation().getPath()
							.replace("/bin/", "/") + "testJava/data/model/",
