...
public class asi extends manager.MutatorUtils implements manager.IMutatorExecutor {
	...
	private ArrayList<Mutator> mutation2(List<EPackage> packages, Resource model, HashMap<String, EObject> hmObjects,
		...
		RandomTypeSelection rts = new RandomTypeSelection(packages, model, "Activity", referenceSelection,
				containerSelection);
	...
	private ArrayList<Mutator> mutation3(List<EPackage> packages, Resource model, HashMap<String, EObject> hmObjects,
		...
		if (objectSelection != null && objectSelection.getObject() != null) {
			if (hmObjects.get("a1") != null) {
				...
				refs.add(new SpecificReferenceConfigurationStrategy(model, objectSelection.getObject(),
						hmObjects.get("a1"), "activities", true));
		...
		if (objectSelection != null && objectSelection.getObject() != null) {
			if (hmObjects.get("a1") != null) {
				...
				refs.add(new SpecificReferenceConfigurationStrategy(model, objectSelection.getObject(),
						hmObjects.get("a1"), "activities", false));
				refsList.put("activities", refs);
			} else {
				return mutations;
			}
	...
	public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics,
		...
		String ecoreURI = "C:/eclipse/workspace/asi/data/model/bpel.ecore";
		String modelURI = "C:/eclipse/workspace/asi/data/model/";
		String modelsURI = "C:/eclipse/workspace/asi/data/out/";
		...
		for (String modelFilename : modelFilenames) {
			...
			Resource mutatormodel = ModelManager.loadModel(mutatorpackages,
					URI.createURI("file:/C:/eclipse/workspace/asi/data/out/asi.model").toFileString());
			...
			for (int i = 0; i < numMutants; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> ls219 = mutation1(packages, model, hashmapEObject, hashmapList);
						if (ls219 != null) {
							int k = 0;
							for (Mutator mut : ls219) {
								if (mut != null) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> la120 = mutation2(packages, model, hashmapEObject, hashmapList);
						if (la120 != null) {
							int k = 0;
							for (Mutator mut : la120) {
								if (mut != null) {
									Object mutated = mut.mutate();
									if (mutated != null) {
										if (mutated instanceof EObject) {
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/asi/data/out/",
					"C:/eclipse/workspace/asi/data/model/bpel.ecore", "C:/eclipse/workspace/asi/data/model/",
					"asi.java", hashmapMutVersions);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/asi/data/out/",
					"C:/eclipse/workspace/asi/data/model/bpel.ecore", "C:/eclipse/workspace/asi/data/model/",
					"asi.java", hashmapMutVersions);
			metricsGenerator.run();
			monitor.worked(1);
