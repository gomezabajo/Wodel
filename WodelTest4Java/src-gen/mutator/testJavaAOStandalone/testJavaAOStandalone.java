package mutator.testJavaAOStandalone;
...
public class testJavaAOStandalone extends MutatorUtils {
	...
	private int mutation1(List<EPackage> packages, Resource model,
		...
		for (EObject object : objects) {
			String modelsFolder = ModelManager.getModelsFolder(testJavaAOStandalone.class);
			...
			if (object != null) {
				...
				if (mutator != null) {
					...
					boolean isRepeated = registryMutantWithBlocksStandalone(ecoreURI, packages, registeredPackages,
							...
							testJavaAOStandalone.class, true, false);
					if (isRepeated == false) {
						numMutantsGenerated++;
						monitor.worked(1);
						k[0] = k[0] + 1;
	...
	public int block_aorbp(int maxAttempts, int numMutants, boolean registry, List<EPackage> packages,
		...
		String modelURI = testJavaAOStandalone.class.getProtectionDomain().getCodeSource().getLocation().getPath()
				.replace("/bin/", "/") + "data/model/";
		String modelsURI = testJavaAOStandalone.class.getProtectionDomain().getCodeSource().getLocation().getPath()
		...
		for (String modelFilename : modelFilenames) {
			...
			numMutants = -1;
			String xmiFilename = testJavaAOStandalone.class.getProtectionDomain().getCodeSource().getLocation()
					.getPath().replace("/bin/", "/") + "data/out/testJavaAO.model";
		...
		for (EObject object : objects) {
			...
			if (object != null) {
				...
				if (mut != null) {
					Object mutated = mut.mutate();
					if (mutated != null) {
						AppMutation appMut = registry13(mut, hmMutator, seed, obSelection.getModel(), mutPaths,
								packages);
						if (appMut != null) {
							muts.getMuts().add(appMut);
						}
	...
	public MutationResults execute(int maxAttempts, int numMutants, boolean registry, boolean metrics,
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, testJavaAOStandalone.class
					.getProtectionDomain().getCodeSource().getLocation().getPath().replace("/bin/", "/") + "data/out/",
					"/WodelTest4Java/data/model/java.ecore",
					testJavaAOStandalone.class.getProtectionDomain().getCodeSource().getLocation().getPath()
							.replace("/bin/", "/") + "data/model/",
					"testJavaAO.mutator", hashmapMutVersions, testJavaAOStandalone.class);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, testJavaAOStandalone.class
					.getProtectionDomain().getCodeSource().getLocation().getPath().replace("/bin/", "/") + "data/out/",
					"/WodelTest4Java/data/model/java.ecore",
					testJavaAOStandalone.class.getProtectionDomain().getCodeSource().getLocation().getPath()
							.replace("/bin/", "/") + "data/model/",
