		...
		for (String modelFilename : modelFilenames) {
			...
			for (int i = 0; i < numMutantsToGenerate; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						List<Mutator> l14 = mutation2(packages, model, hashmapEObject, hashmapList, serialize, test,
								classes);
						if (l14 != null) {
							for (Mutator mut : l14) {
								if (mut != null) {
		...
		for (String modelFilename : modelFilenames) {
			...
			for (int i = 0; i < numMutantsToGenerate; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						List<Mutator> l15 = mutation3(packages, model, hashmapEObject, hashmapList, serialize, test,
								classes);
						if (l15 != null) {
							for (Mutator mut : l15) {
								if (mut != null) {
