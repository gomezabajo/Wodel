...
public class ia1 extends manager.MutatorUtils implements manager.IMutatorExecutor {
	...
	private ArrayList<Mutator> mutation2(List<EPackage> packages, Resource model, HashMap<String, EObject> hmObjects,
		...
		if (hmObjects.get("op") != null) {
			refSelection7 = new SpecificObjectSelection(packages, model, hmObjects.get("op"), "right");
		} else {
			if (hmList.get("op") != null) {
				refSelection7 = new SpecificObjectSelection(packages, model, hmList.get("op"), "right");
			} else {
				return mutations;
			}
		}
		refs.put("left", refSelection7);
	...
	public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics,
		...
		for (String modelFilename : modelFilenames) {
			...
			Resource mutatormodel = ModelManager.loadModel(mutatorpackages,
					URI.createURI("file:/C:/eclipse/workspace/ia1/data/out/ia1.model").toFileString());
			...
			for (int i = 0; i < numMutants; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> lop62 = mutation1(packages, model, hashmapEObject, hashmapList);
						if (lop62 != null) {
							int k = 0;
							for (Mutator mut : lop62) {
								if (mut != null) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> lexp63 = mutation2(packages, model, hashmapEObject, hashmapList);
						if (lexp63 != null) {
							int k = 0;
							for (Mutator mut : lexp63) {
								if (mut != null) {
									Object mutated = mut.mutate();
									if (mutated != null) {
										if (mutated instanceof EObject) {
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/ia1/data/out/",
					"C:/eclipse/workspace/ia1/data/model/Asple.ecore", "C:/eclipse/workspace/ia1/data/model/",
					"ia1.java", hashmapMutVersions);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/ia1/data/out/",
					"C:/eclipse/workspace/ia1/data/model/Asple.ecore", "C:/eclipse/workspace/ia1/data/model/",
					"ia1.java", hashmapMutVersions);
			metricsGenerator.run();
			monitor.worked(1);
