		...
		for (String modelFilename : modelFilenames) {
			...
			for (int i = 0; i < numMutantsToGenerate; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						List<Mutator> l58 = mutation1(packages, model, hashmapEObject, hashmapList, serialize, test,
								classes);
						if (l58 != null) {
							for (Mutator mut : l58) {
								if (mut != null) {
		...
		for (String modelFilename : modelFilenames) {
			...
			for (int i = 0; i < numMutantsToGenerate; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						List<Mutator> l59 = mutation2(packages, model, hashmapEObject, hashmapList, serialize, test,
								classes);
						if (l59 != null) {
							for (Mutator mut : l59) {
								if (mut != null) {
		...
		for (String modelFilename : modelFilenames) {
			...
			for (int i = 0; i < numMutantsToGenerate; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						List<Mutator> l60 = mutation3(packages, model, hashmapEObject, hashmapList, serialize, test,
								classes);
						if (l60 != null) {
							for (Mutator mut : l60) {
								if (mut != null) {
