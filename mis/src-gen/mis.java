...
public class mis extends manager.MutatorUtils implements manager.IMutatorExecutor {
	...
	public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics,
		...
		for (String modelFilename : modelFilenames) {
			...
			Resource mutatormodel = ModelManager.loadModel(mutatorpackages,
					URI.createURI("file:/C:/eclipse/workspace/mis/data/out/mis.model").toFileString());
			...
			for (int i = 0; i < numMutants; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> ls0143 = mutation1(packages, model, hashmapEObject, hashmapList);
						if (ls0143 != null) {
							int k = 0;
							for (Mutator mut : ls0143) {
								if (mut != null) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> ls1144 = mutation2(packages, model, hashmapEObject, hashmapList);
						if (ls1144 != null) {
							int k = 0;
							for (Mutator mut : ls1144) {
								if (mut != null) {
									Object mutated = mut.mutate();
									if (mutated != null) {
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/mis/data/out/",
					"C:/eclipse/workspace/mis/data/model/DFAAutomaton.ecore", "C:/eclipse/workspace/mis/data/model/",
					"mis.java", hashmapMutVersions);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/mis/data/out/",
					"C:/eclipse/workspace/mis/data/model/DFAAutomaton.ecore", "C:/eclipse/workspace/mis/data/model/",
					"mis.java", hashmapMutVersions);
			metricsGenerator.run();
			monitor.worked(1);
