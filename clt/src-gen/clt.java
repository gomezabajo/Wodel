...
public class clt extends manager.MutatorUtils implements manager.IMutatorExecutor {
	...
	public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics,
		...
		for (String modelFilename : modelFilenames) {
			...
			Resource mutatormodel = ModelManager.loadModel(mutatorpackages,
					URI.createURI("file:/C:/eclipse/workspace/clt/data/out/clt.model").toFileString());
			...
			for (int i = 0; i < numMutants; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> l226 = mutation1(packages, model, hashmapEObject, hashmapList);
						if (l226 != null) {
							int k = 0;
							for (Mutator mut : l226) {
								if (mut != null) {
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/clt/data/out/",
					"C:/eclipse/workspace/clt/data/model/DFAAutomaton.ecore", "C:/eclipse/workspace/clt/data/model/",
					"clt.java", hashmapMutVersions);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/clt/data/out/",
					"C:/eclipse/workspace/clt/data/model/DFAAutomaton.ecore", "C:/eclipse/workspace/clt/data/model/",
					"clt.java", hashmapMutVersions);
			metricsGenerator.run();
			monitor.worked(1);
