...
public class sst extends manager.MutatorUtils implements manager.IMutatorExecutor {
	...
	private ArrayList<Mutator> mutation2(List<EPackage> packages, Resource model, HashMap<String, EObject> hmObjects,
		...
		((ReferenceEvaluation) exp0.first).value = new SpecificObjectSelection(packages, model, hmObjects.get("t"))
				.getObject();
		...
		if (objectSelection != null && objectSelection.getObject() != null) {
			if (hmObjects.get("t") != null) {
				...
				refs.add(new SwapReferenceConfigurationStrategy(objectSelection.getObject(), hmObjects.get("t"),
						"symbol", "symbol"));
	...
	public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics,
		...
		for (String modelFilename : modelFilenames) {
			...
			Resource mutatormodel = ModelManager.loadModel(mutatorpackages,
					URI.createURI("file:/C:/eclipse/workspace/sst/data/out/sst.model").toFileString());
			...
			for (int i = 0; i < numMutants; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> lt173 = mutation1(packages, model, hashmapEObject, hashmapList);
						if (lt173 != null) {
							int k = 0;
							for (Mutator mut : lt173) {
								if (mut != null) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> l174 = mutation2(packages, model, hashmapEObject, hashmapList);
						if (l174 != null) {
							int k = 0;
							for (Mutator mut : l174) {
								if (mut != null) {
									Object mutated = mut.mutate();
									if (mutated != null) {
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/sst/data/out/",
					"C:/eclipse/workspace/sst/data/model/DFAAutomaton.ecore", "C:/eclipse/workspace/sst/data/model/",
					"sst.java", hashmapMutVersions);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/sst/data/out/",
					"C:/eclipse/workspace/sst/data/model/DFAAutomaton.ecore", "C:/eclipse/workspace/sst/data/model/",
					"sst.java", hashmapMutVersions);
			metricsGenerator.run();
			monitor.worked(1);
