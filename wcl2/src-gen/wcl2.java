...
public class wcl2 extends manager.MutatorUtils implements manager.IMutatorExecutor {
	private ArrayList<Mutator> mutation1(List<EPackage> packages, Resource model, HashMap<String, EObject> hmObjects,
		...
		RandomTypeSelection refRts48 = new RandomTypeSelection(packages, model, "Association");
		EObject refObject48 = refRts48.getObject();
		ObSelectionStrategy refSelection48 = null;
		if (refObject48 != null) {
			refSelection48 = new SpecificObjectSelection(packages, model, refObject48);
		}
		refs.put("association", refSelection48);
		...
		if (objectSelection != null && objectSelection.getObject() != null) {
			mut = new RetypeObjectMutator(model, packages, objectSelection.getObject(), referenceSelection,
					containerSelection, atts, refs, "AssociationClass");
		}
		if (mut != null) {
			mut.setId("m1");
			mutations.add(mut);
	...
	public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics,
		...
		String ecoreURI = "C:/eclipse/workspace/wcl2/data/model/UMLClassDiagram.ecore";
		String modelURI = "C:/eclipse/workspace/wcl2/data/model/";
		String modelsURI = "C:/eclipse/workspace/wcl2/data/out/";
		...
		for (String modelFilename : modelFilenames) {
			...
			Resource mutatormodel = ModelManager.loadModel(mutatorpackages,
					URI.createURI("file:/C:/eclipse/workspace/wcl2/data/out/wcl2.model").toFileString());
			...
			for (int i = 0; i < numMutants; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> l364 = mutation1(packages, model, hashmapEObject, hashmapList);
						if (l364 != null) {
							int k = 0;
							for (Mutator mut : l364) {
								if (mut != null) {
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/wcl2/data/out/",
					"C:/eclipse/workspace/wcl2/data/model/UMLClassDiagram.ecore",
					"C:/eclipse/workspace/wcl2/data/model/", "wcl2.java", hashmapMutVersions);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/wcl2/data/out/",
					"C:/eclipse/workspace/wcl2/data/model/UMLClassDiagram.ecore",
					"C:/eclipse/workspace/wcl2/data/model/", "wcl2.java", hashmapMutVersions);
			metricsGenerator.run();
			monitor.worked(1);
