...
public class ajc extends manager.MutatorUtils implements manager.IMutatorExecutor {
	private ArrayList<Mutator> mutation1(List<EPackage> packages, Resource model, HashMap<String, EObject> hmObjects,
		...
		((AttributeEvaluation) exp0.first).name = "suppressJoinFailure";
	...
	public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics,
		...
		String ecoreURI = "C:/eclipse/workspace/ajc/data/model/bpel.ecore";
		String modelURI = "C:/eclipse/workspace/ajc/data/model/";
		String modelsURI = "C:/eclipse/workspace/ajc/data/out/";
		...
		for (String modelFilename : modelFilenames) {
			...
			Resource mutatormodel = ModelManager.loadModel(mutatorpackages,
					URI.createURI("file:/C:/eclipse/workspace/ajc/data/out/ajc.model").toFileString());
			...
			for (int i = 0; i < numMutants; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> l309 = mutation1(packages, model, hashmapEObject, hashmapList);
						if (l309 != null) {
							int k = 0;
							for (Mutator mut : l309) {
								if (mut != null) {
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/ajc/data/out/",
					"C:/eclipse/workspace/ajc/data/model/bpel.ecore", "C:/eclipse/workspace/ajc/data/model/",
					"ajc.java", hashmapMutVersions);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/ajc/data/out/",
					"C:/eclipse/workspace/ajc/data/model/bpel.ecore", "C:/eclipse/workspace/ajc/data/model/",
					"ajc.java", hashmapMutVersions);
			metricsGenerator.run();
			monitor.worked(1);
