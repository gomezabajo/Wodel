package mutator.testLCStandalone;
...
public class testLCStandalone extends MutatorUtils {
	...
	private static AppMutation registry1(Mutator mut, Map<String, EObject> hmMutator, Resource seed, Resource mutant,
		...
		EObject foundObject = findEObjectForRegistry(seed, mutant, mut.getRemovedObject(), mut.getObjectByID(),
	...
	public int block_mtg(int maxAttempts, int numMutants, boolean registry, List<EPackage> packages,
		...
		String modelURI = testLCStandalone.class.getProtectionDomain().getCodeSource().getLocation().getPath()
				.replace("/bin/", "/") + "data/model/";
		String modelsURI = testLCStandalone.class.getProtectionDomain().getCodeSource().getLocation().getPath()
		...
		for (String modelFilename : modelFilenames) {
			...
			numMutants = 2;
			String xmiFilename = testLCStandalone.class.getProtectionDomain().getCodeSource().getLocation().getPath()
			...
			for (int i = 0; i < numMutantsToGenerate; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					isRepeated = registryMutantWithBlocksStandalone(ecoreURI, packages, registeredPackages,
							...
							mutantIndex, mutPaths, hashmapMutVersions, "WodelTest4LC", serialize, test, classes,
							testLCStandalone.class, true, false);
					if (isRepeated == false) {
						numMutantsGenerated++;
						k[0] = k[0] + 1;
					}
	...
	private static AppMutation registry5(Mutator mut, Map<String, EObject> hmMutator, Resource seed, Resource mutant,
		...
		refMut0.setFrom(mutator.getPrevious("src"));
		refMut0.setTo(mutator.getNext("src"));
	...
	public MutationResults execute(int maxAttempts, int numMutants, boolean registry, boolean metrics,
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(
					metricspackages, testLCStandalone.class.getProtectionDomain().getCodeSource().getLocation()
