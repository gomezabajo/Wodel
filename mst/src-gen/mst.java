...
public class mst extends manager.MutatorUtils implements manager.IMutatorExecutor {
	private ArrayList<Mutator> mutation1(List<EPackage> packages, Resource model, HashMap<String, EObject> hmObjects,
		...
		ModifyTargetReferenceMutator mut = new ModifyTargetReferenceMutator(model, packages, sourceSelection,
				newTargetSelection, "symbol");
		if (mut != null) {
			mut.setId("m1");
			mutations.add(mut);
		}
	...
	public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics,
		...
		for (String modelFilename : modelFilenames) {
			...
			Resource mutatormodel = ModelManager.loadModel(mutatorpackages,
					URI.createURI("file:/C:/eclipse/workspace/mst/data/out/mst.model").toFileString());
			...
			for (int i = 0; i < numMutants; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> l114 = mutation1(packages, model, hashmapEObject, hashmapList);
						if (l114 != null) {
							int k = 0;
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/mst/data/out/",
					"C:/eclipse/workspace/mst/data/model/DFAAutomaton.ecore", "C:/eclipse/workspace/mst/data/model/",
					"mst.java", hashmapMutVersions);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/mst/data/out/",
					"C:/eclipse/workspace/mst/data/model/DFAAutomaton.ecore", "C:/eclipse/workspace/mst/data/model/",
					"mst.java", hashmapMutVersions);
			metricsGenerator.run();
			monitor.worked(1);
