package mutator.testATLOPEStandalone;
...
public class testATLOPEStandalone extends MutatorUtils {
	...
	private static List<Mutator> mutation1(List<EPackage> packages, Resource model,
		...
		String absoluteMetamodelPath = testATLOPEStandalone.class.getProtectionDomain().getCodeSource().getLocation()
				.getPath().replace("/bin/", "/") + relativeMetamodelPath;
		resourcePackages = ModelManager.loadMetaModel(absoluteMetamodelPath, testATLOPEStandalone.class);
		resources = new ArrayList<Resource>();
		String relativeEcoreURI_0 = "data/model/out/Persons.ecore";
		String absoluteEcoreURI_0 = testATLOPEStandalone.class.getProtectionDomain().getCodeSource().getLocation()
		...
		String absoluteEcoreURI_1 = testATLOPEStandalone.class.getProtectionDomain().getCodeSource().getLocation()
		...
		String absoluteObjectMetamodelPath = testATLOPEStandalone.class.getProtectionDomain().getCodeSource()
				.getLocation().getPath().replace("/bin/", "/") + relativeObjectMetamodelPath;
		objectPackages = ModelManager.loadMetaModel(absoluteObjectMetamodelPath, testATLOPEStandalone.class);
	...
	private static List<Mutator> mutation5(List<EPackage> packages, Resource model,
		...
		SimpleEntry<EObject, SimpleEntry<Resource, List<EPackage>>> entry_ope_4 = hmObjects.get("ope");
		if (entry_ope_4 != null) {
			refSelection4 = new SpecificObjectSelection(packages, model, entry_ope_4.getKey());
		} else {
			List<SimpleEntry<EObject, SimpleEntry<Resource, List<EPackage>>>> listEntry_ope_4 = hmList.get("ope");
			if (listEntry_ope_4 != null) {
				List<EObject> objs = new ArrayList<EObject>();
				for (SimpleEntry<EObject, SimpleEntry<Resource, List<EPackage>>> ent : listEntry_ope_4) {
	...
	public int block_cope(int maxAttempts, int numMutants, boolean registry, List<EPackage> packages,
		...
		String modelURI = testATLOPEStandalone.class.getProtectionDomain().getCodeSource().getLocation().getPath()
				.replace("/bin/", "/") + "data/model/";
		String modelsURI = testATLOPEStandalone.class.getProtectionDomain().getCodeSource().getLocation().getPath()
		...
		for (String modelFilename : modelFilenames) {
			...
			numMutants = 2;
			String xmiFilename = testATLOPEStandalone.class.getProtectionDomain().getCodeSource().getLocation()
					.getPath().replace("/bin/", "/") + "data/out/testATLOPE.model";
			...
			for (int i = 0; i < numMutantsToGenerate; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						List<Mutator> lcl103 = mutation1(packages, model, hashmapEObject, hashmapList, serialize, test,
								classes);
						if (lcl103 != null) {
							for (Mutator mut : lcl103) {
								if (mut != null) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						List<Mutator> lp104 = mutation2(packages, model, hashmapEObject, hashmapList, serialize, test,
								classes);
						if (lp104 != null) {
							for (Mutator mut : lp104) {
								if (mut != null) {
									Object mutated = mut.mutate();
									if (mutated != null) {
										if (mutated instanceof EObject) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						List<Mutator> lmod105 = mutation3(packages, model, hashmapEObject, hashmapList, serialize, test,
								classes);
						if (lmod105 != null) {
							for (Mutator mut : lmod105) {
								if (mut != null) {
									Object mutated = mut.mutate();
									if (mutated != null) {
										if (mutated instanceof EObject) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						List<Mutator> lope106 = mutation4(packages, model, hashmapEObject, hashmapList, serialize, test,
								classes);
						if (lope106 != null) {
							for (Mutator mut : lope106) {
								if (mut != null) {
									Object mutated = mut.mutate();
									if (mutated != null) {
										if (mutated instanceof EObject) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						List<Mutator> lelem107 = mutation5(packages, model, hashmapEObject, hashmapList, serialize,
								test, classes);
						if (lelem107 != null) {
							for (Mutator mut : lelem107) {
								if (mut != null) {
									Object mutated = mut.mutate();
									if (mutated != null) {
										if (mutated instanceof EObject) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						List<Mutator> l108 = mutation6(packages, model, hashmapEObject, hashmapList, serialize, test,
								classes);
						if (l108 != null) {
							for (Mutator mut : l108) {
								if (mut != null) {
									Object mutated = mut.mutate();
									if (mutated != null) {
										String mutatorPath = mutPath + "/Output" + i + "_" + j + "_" + k[0]
					...
					isRepeated = registryMutantWithBlocksStandalone(ecoreURI, packages, registeredPackages,
							...
							testATLOPEStandalone.class, true, false);
					if (isRepeated == false) {
						numMutantsGenerated++;
						k[0] = k[0] + 1;
					}
		...
		for (String modelFilename : modelFilenames) {
			...
			for (int i = 0; i < numMutantsToGenerate; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						List<Mutator> l109 = mutation7(packages, model, hashmapEObject, hashmapList, serialize, test,
								classes);
						if (l109 != null) {
							for (Mutator mut : l109) {
								if (mut != null) {
									Object mutated = mut.mutate();
									if (mutated != null) {
										String mutatorPath = mutPath + "/Output" + i + "_" + j + "_" + k[0]
		...
		for (String modelFilename : modelFilenames) {
			...
			for (int i = 0; i < numMutantsToGenerate; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						List<Mutator> lp110 = mutation8(packages, model, hashmapEObject, hashmapList, serialize, test,
								classes);
						if (lp110 != null) {
							for (Mutator mut : lp110) {
								if (mut != null) {
									Object mutated = mut.mutate();
									if (mutated != null) {
										if (mutated instanceof EObject) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						List<Mutator> ltype111 = mutation9(packages, model, hashmapEObject, hashmapList, serialize,
								test, classes);
						if (ltype111 != null) {
							for (Mutator mut : ltype111) {
								if (mut != null) {
									Object mutated = mut.mutate();
									if (mutated != null) {
										if (mutated instanceof EObject) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						List<Mutator> lcl112 = mutation10(packages, model, hashmapEObject, hashmapList, serialize, test,
								classes);
						if (lcl112 != null) {
							for (Mutator mut : lcl112) {
								if (mut != null) {
									Object mutated = mut.mutate();
									if (mutated != null) {
										if (mutated instanceof EObject) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						List<Mutator> l113 = mutation11(packages, model, hashmapEObject, hashmapList, serialize, test,
								classes);
						if (l113 != null) {
		...
		for (String modelFilename : modelFilenames) {
			...
			for (int i = 0; i < numMutantsToGenerate; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						List<Mutator> l114 = mutation12(packages, model, hashmapEObject, hashmapList, serialize, test,
								classes);
						if (l114 != null) {
	...
	public MutationResults execute(int maxAttempts, int numMutants, boolean registry, boolean metrics,
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, testATLOPEStandalone.class
					.getProtectionDomain().getCodeSource().getLocation().getPath().replace("/bin/", "/") + "data/out/",
					"/WodelTest4ATL/data/model/ATL.ecore",
					testATLOPEStandalone.class.getProtectionDomain().getCodeSource().getLocation().getPath()
							.replace("/bin/", "/") + "data/model/",
					"testATLOPE.mutator", hashmapMutVersions, testATLOPEStandalone.class);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, testATLOPEStandalone.class
					.getProtectionDomain().getCodeSource().getLocation().getPath().replace("/bin/", "/") + "data/out/",
					"/WodelTest4ATL/data/model/ATL.ecore",
					testATLOPEStandalone.class.getProtectionDomain().getCodeSource().getLocation().getPath()
							.replace("/bin/", "/") + "data/model/",
