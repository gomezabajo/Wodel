...
public class wge extends manager.MutatorUtils implements manager.IMutatorExecutor {
	private ArrayList<Mutator> mutation1(List<EPackage> packages, Resource model, HashMap<String, EObject> hmObjects,
		...
		if (objectSelection != null && objectSelection.getObject() != null) {
			...
			refs.add(new RandomReferenceConfigurationStrategy(model, objectSelection.getObject(), "super", "Class"));
	...
	public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics,
		...
		String ecoreURI = "C:/eclipse/workspace/wge/data/model/UMLClassDiagram.ecore";
		String modelURI = "C:/eclipse/workspace/wge/data/model/";
		String modelsURI = "C:/eclipse/workspace/wge/data/out/";
		...
		for (String modelFilename : modelFilenames) {
			...
			Resource mutatormodel = ModelManager.loadModel(mutatorpackages,
					URI.createURI("file:/C:/eclipse/workspace/wge/data/out/wge.model").toFileString());
			...
			for (int i = 0; i < numMutants; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> l396 = mutation1(packages, model, hashmapEObject, hashmapList);
						if (l396 != null) {
							int k = 0;
							for (Mutator mut : l396) {
								if (mut != null) {
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/wge/data/out/",
					"C:/eclipse/workspace/wge/data/model/UMLClassDiagram.ecore", "C:/eclipse/workspace/wge/data/model/",
					"wge.java", hashmapMutVersions);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/wge/data/out/",
					"C:/eclipse/workspace/wge/data/model/UMLClassDiagram.ecore", "C:/eclipse/workspace/wge/data/model/",
					"wge.java", hashmapMutVersions);
			metricsGenerator.run();
			monitor.worked(1);
