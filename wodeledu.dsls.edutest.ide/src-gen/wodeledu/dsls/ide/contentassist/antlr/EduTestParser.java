/*
 * generated by Xtext 2.12.0
 */
package wodeledu.dsls.ide.contentassist.antlr;

import com.google.inject.Inject;
import java.util.HashMap;
import java.util.Map;
import org.eclipse.xtext.AbstractElement;
import org.eclipse.xtext.ide.editor.contentassist.antlr.AbstractContentAssistParser;
import wodeledu.dsls.ide.contentassist.antlr.internal.InternalEduTestParser;
import wodeledu.dsls.services.EduTestGrammarAccess;

public class EduTestParser extends AbstractContentAssistParser {

	@Inject
	private EduTestGrammarAccess grammarAccess;

	private Map<AbstractElement, String> nameMappings;

	@Override
	protected InternalEduTestParser createParser() {
		InternalEduTestParser result = new InternalEduTestParser(null);
		result.setGrammarAccess(grammarAccess);
		return result;
	}

	@Override
	protected String getRuleName(AbstractElement element) {
		if (nameMappings == null) {
			nameMappings = new HashMap<AbstractElement, String>() {
				private static final long serialVersionUID = 1L;
				{
					put(grammarAccess.getMutatorTestsAccess().getAlternatives(), "rule__MutatorTests__Alternatives");
					put(grammarAccess.getTestConfigurationAccess().getRetryAlternatives_3_0(), "rule__TestConfiguration__RetryAlternatives_3_0");
					put(grammarAccess.getMultiChoiceEmConfigAccess().getRetryAlternatives_3_0(), "rule__MultiChoiceEmConfig__RetryAlternatives_3_0");
					put(grammarAccess.getMultiChoiceEmConfigAccess().getWeightedAlternatives_7_0(), "rule__MultiChoiceEmConfig__WeightedAlternatives_7_0");
					put(grammarAccess.getTextConfigurationAccess().getRetryAlternatives_3_0(), "rule__TextConfiguration__RetryAlternatives_3_0");
					put(grammarAccess.getEStringAccess().getAlternatives(), "rule__EString__Alternatives");
					put(grammarAccess.getEDoubleAccess().getAlternatives_4_0(), "rule__EDouble__Alternatives_4_0");
					put(grammarAccess.getOrderAccess().getAlternatives(), "rule__Order__Alternatives");
					put(grammarAccess.getModeAccess().getAlternatives(), "rule__Mode__Alternatives");
					put(grammarAccess.getNavigationAccess().getAlternatives(), "rule__Navigation__Alternatives");
					put(grammarAccess.getProgramAccess().getGroup(), "rule__Program__Group__0");
					put(grammarAccess.getAlternativeResponseAccess().getGroup(), "rule__AlternativeResponse__Group__0");
					put(grammarAccess.getAlternativeResponseAccess().getGroup_1(), "rule__AlternativeResponse__Group_1__0");
					put(grammarAccess.getAlternativeResponseAccess().getGroup_1_1(), "rule__AlternativeResponse__Group_1_1__0");
					put(grammarAccess.getMultiChoiceDiagramAccess().getGroup(), "rule__MultiChoiceDiagram__Group__0");
					put(grammarAccess.getMultiChoiceDiagramAccess().getGroup_1(), "rule__MultiChoiceDiagram__Group_1__0");
					put(grammarAccess.getMultiChoiceDiagramAccess().getGroup_1_1(), "rule__MultiChoiceDiagram__Group_1_1__0");
					put(grammarAccess.getMultiChoiceEmendationAccess().getGroup(), "rule__MultiChoiceEmendation__Group__0");
					put(grammarAccess.getMultiChoiceEmendationAccess().getGroup_1(), "rule__MultiChoiceEmendation__Group_1__0");
					put(grammarAccess.getMultiChoiceEmendationAccess().getGroup_1_1(), "rule__MultiChoiceEmendation__Group_1_1__0");
					put(grammarAccess.getMatchPairsAccess().getGroup(), "rule__MatchPairs__Group__0");
					put(grammarAccess.getMatchPairsAccess().getGroup_1(), "rule__MatchPairs__Group_1__0");
					put(grammarAccess.getMatchPairsAccess().getGroup_1_1(), "rule__MatchPairs__Group_1_1__0");
					put(grammarAccess.getMissingWordsAccess().getGroup(), "rule__MissingWords__Group__0");
					put(grammarAccess.getMissingWordsAccess().getGroup_1(), "rule__MissingWords__Group_1__0");
					put(grammarAccess.getMissingWordsAccess().getGroup_1_1(), "rule__MissingWords__Group_1_1__0");
					put(grammarAccess.getMultiChoiceTextAccess().getGroup(), "rule__MultiChoiceText__Group__0");
					put(grammarAccess.getMultiChoiceTextAccess().getGroup_1(), "rule__MultiChoiceText__Group_1__0");
					put(grammarAccess.getMultiChoiceTextAccess().getGroup_1_1(), "rule__MultiChoiceText__Group_1_1__0");
					put(grammarAccess.getAlternativeTextAccess().getGroup(), "rule__AlternativeText__Group__0");
					put(grammarAccess.getAlternativeTextAccess().getGroup_1(), "rule__AlternativeText__Group_1__0");
					put(grammarAccess.getAlternativeTextAccess().getGroup_1_1(), "rule__AlternativeText__Group_1_1__0");
					put(grammarAccess.getDragAndDropTextAccess().getGroup(), "rule__DragAndDropText__Group__0");
					put(grammarAccess.getDragAndDropTextAccess().getGroup_1(), "rule__DragAndDropText__Group_1__0");
					put(grammarAccess.getDragAndDropTextAccess().getGroup_1_1(), "rule__DragAndDropText__Group_1_1__0");
					put(grammarAccess.getProgramConfigurationAccess().getGroup(), "rule__ProgramConfiguration__Group__0");
					put(grammarAccess.getTestConfigurationAccess().getGroup(), "rule__TestConfiguration__Group__0");
					put(grammarAccess.getMultiChoiceEmConfigAccess().getGroup(), "rule__MultiChoiceEmConfig__Group__0");
					put(grammarAccess.getTextConfigurationAccess().getGroup(), "rule__TextConfiguration__Group__0");
					put(grammarAccess.getTestAccess().getGroup(), "rule__Test__Group__0");
					put(grammarAccess.getTestAccess().getGroup_5(), "rule__Test__Group_5__0");
					put(grammarAccess.getTestAccess().getGroup_5_1(), "rule__Test__Group_5_1__0");
					put(grammarAccess.getEDoubleAccess().getGroup(), "rule__EDouble__Group__0");
					put(grammarAccess.getEDoubleAccess().getGroup_4(), "rule__EDouble__Group_4__0");
					put(grammarAccess.getProgramAccess().getConfigAssignment_1(), "rule__Program__ConfigAssignment_1");
					put(grammarAccess.getProgramAccess().getExercisesAssignment_2(), "rule__Program__ExercisesAssignment_2");
					put(grammarAccess.getAlternativeResponseAccess().getBlocksAssignment_1_0(), "rule__AlternativeResponse__BlocksAssignment_1_0");
					put(grammarAccess.getAlternativeResponseAccess().getBlocksAssignment_1_1_1(), "rule__AlternativeResponse__BlocksAssignment_1_1_1");
					put(grammarAccess.getAlternativeResponseAccess().getConfigAssignment_3(), "rule__AlternativeResponse__ConfigAssignment_3");
					put(grammarAccess.getAlternativeResponseAccess().getTestsAssignment_4(), "rule__AlternativeResponse__TestsAssignment_4");
					put(grammarAccess.getMultiChoiceDiagramAccess().getBlocksAssignment_1_0(), "rule__MultiChoiceDiagram__BlocksAssignment_1_0");
					put(grammarAccess.getMultiChoiceDiagramAccess().getBlocksAssignment_1_1_1(), "rule__MultiChoiceDiagram__BlocksAssignment_1_1_1");
					put(grammarAccess.getMultiChoiceDiagramAccess().getConfigAssignment_3(), "rule__MultiChoiceDiagram__ConfigAssignment_3");
					put(grammarAccess.getMultiChoiceDiagramAccess().getTestsAssignment_4(), "rule__MultiChoiceDiagram__TestsAssignment_4");
					put(grammarAccess.getMultiChoiceEmendationAccess().getBlocksAssignment_1_0(), "rule__MultiChoiceEmendation__BlocksAssignment_1_0");
					put(grammarAccess.getMultiChoiceEmendationAccess().getBlocksAssignment_1_1_1(), "rule__MultiChoiceEmendation__BlocksAssignment_1_1_1");
					put(grammarAccess.getMultiChoiceEmendationAccess().getConfigAssignment_3(), "rule__MultiChoiceEmendation__ConfigAssignment_3");
					put(grammarAccess.getMultiChoiceEmendationAccess().getTestsAssignment_4(), "rule__MultiChoiceEmendation__TestsAssignment_4");
					put(grammarAccess.getMatchPairsAccess().getBlocksAssignment_1_0(), "rule__MatchPairs__BlocksAssignment_1_0");
					put(grammarAccess.getMatchPairsAccess().getBlocksAssignment_1_1_1(), "rule__MatchPairs__BlocksAssignment_1_1_1");
					put(grammarAccess.getMatchPairsAccess().getConfigAssignment_3(), "rule__MatchPairs__ConfigAssignment_3");
					put(grammarAccess.getMatchPairsAccess().getTestsAssignment_4(), "rule__MatchPairs__TestsAssignment_4");
					put(grammarAccess.getMissingWordsAccess().getBlocksAssignment_1_0(), "rule__MissingWords__BlocksAssignment_1_0");
					put(grammarAccess.getMissingWordsAccess().getBlocksAssignment_1_1_1(), "rule__MissingWords__BlocksAssignment_1_1_1");
					put(grammarAccess.getMissingWordsAccess().getConfigAssignment_3(), "rule__MissingWords__ConfigAssignment_3");
					put(grammarAccess.getMissingWordsAccess().getTestsAssignment_4(), "rule__MissingWords__TestsAssignment_4");
					put(grammarAccess.getMultiChoiceTextAccess().getBlocksAssignment_1_0(), "rule__MultiChoiceText__BlocksAssignment_1_0");
					put(grammarAccess.getMultiChoiceTextAccess().getBlocksAssignment_1_1_1(), "rule__MultiChoiceText__BlocksAssignment_1_1_1");
					put(grammarAccess.getMultiChoiceTextAccess().getConfigAssignment_3(), "rule__MultiChoiceText__ConfigAssignment_3");
					put(grammarAccess.getMultiChoiceTextAccess().getTestsAssignment_4(), "rule__MultiChoiceText__TestsAssignment_4");
					put(grammarAccess.getAlternativeTextAccess().getBlocksAssignment_1_0(), "rule__AlternativeText__BlocksAssignment_1_0");
					put(grammarAccess.getAlternativeTextAccess().getBlocksAssignment_1_1_1(), "rule__AlternativeText__BlocksAssignment_1_1_1");
					put(grammarAccess.getAlternativeTextAccess().getConfigAssignment_3(), "rule__AlternativeText__ConfigAssignment_3");
					put(grammarAccess.getAlternativeTextAccess().getTestsAssignment_4(), "rule__AlternativeText__TestsAssignment_4");
					put(grammarAccess.getDragAndDropTextAccess().getBlocksAssignment_1_0(), "rule__DragAndDropText__BlocksAssignment_1_0");
					put(grammarAccess.getDragAndDropTextAccess().getBlocksAssignment_1_1_1(), "rule__DragAndDropText__BlocksAssignment_1_1_1");
					put(grammarAccess.getDragAndDropTextAccess().getConfigAssignment_3(), "rule__DragAndDropText__ConfigAssignment_3");
					put(grammarAccess.getDragAndDropTextAccess().getTestsAssignment_4(), "rule__DragAndDropText__TestsAssignment_4");
					put(grammarAccess.getProgramConfigurationAccess().getNavigationAssignment_2(), "rule__ProgramConfiguration__NavigationAssignment_2");
					put(grammarAccess.getTestConfigurationAccess().getRetryAssignment_3(), "rule__TestConfiguration__RetryAssignment_3");
					put(grammarAccess.getMultiChoiceEmConfigAccess().getRetryAssignment_3(), "rule__MultiChoiceEmConfig__RetryAssignment_3");
					put(grammarAccess.getMultiChoiceEmConfigAccess().getWeightedAssignment_7(), "rule__MultiChoiceEmConfig__WeightedAssignment_7");
					put(grammarAccess.getMultiChoiceEmConfigAccess().getPenaltyAssignment_11(), "rule__MultiChoiceEmConfig__PenaltyAssignment_11");
					put(grammarAccess.getMultiChoiceEmConfigAccess().getOrderAssignment_15(), "rule__MultiChoiceEmConfig__OrderAssignment_15");
					put(grammarAccess.getMultiChoiceEmConfigAccess().getModeAssignment_19(), "rule__MultiChoiceEmConfig__ModeAssignment_19");
					put(grammarAccess.getTextConfigurationAccess().getRetryAssignment_3(), "rule__TextConfiguration__RetryAssignment_3");
					put(grammarAccess.getTextConfigurationAccess().getIdentifierAssignment_7(), "rule__TextConfiguration__IdentifierAssignment_7");
					put(grammarAccess.getTestAccess().getSourceAssignment_2(), "rule__Test__SourceAssignment_2");
					put(grammarAccess.getTestAccess().getQuestionAssignment_4(), "rule__Test__QuestionAssignment_4");
					put(grammarAccess.getTestAccess().getExpressionAssignment_5_0(), "rule__Test__ExpressionAssignment_5_0");
					put(grammarAccess.getTestAccess().getIdentifierAssignment_5_1_1(), "rule__Test__IdentifierAssignment_5_1_1");
				}
			};
		}
		return nameMappings.get(element);
	}
			
	@Override
	protected String[] getInitialHiddenTokens() {
		return new String[] { "RULE_WS", "RULE_ML_COMMENT", "RULE_SL_COMMENT" };
	}

	public EduTestGrammarAccess getGrammarAccess() {
		return this.grammarAccess;
	}

	public void setGrammarAccess(EduTestGrammarAccess grammarAccess) {
		this.grammarAccess = grammarAccess;
	}
}
