...
public class xee extends manager.MutatorUtils implements manager.IMutatorExecutor {
	...
	private ArrayList<Mutator> mutation2(List<EPackage> packages, Resource model, HashMap<String, EObject> hmObjects,
		...
		RandomTypeSelection rts = new RandomTypeSelection(packages, model, "OnEvent", referenceSelection,
				containerSelection);
	...
	public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics,
		...
		String ecoreURI = "C:/eclipse/workspace/xee/data/model/bpel.ecore";
		String modelURI = "C:/eclipse/workspace/xee/data/model/";
		String modelsURI = "C:/eclipse/workspace/xee/data/out/";
		...
		for (String modelFilename : modelFilenames) {
			...
			Resource mutatormodel = ModelManager.loadModel(mutatorpackages,
					URI.createURI("file:/C:/eclipse/workspace/xee/data/out/xee.model").toFileString());
			...
			for (int i = 0; i < numMutants; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> lev261 = mutation1(packages, model, hashmapEObject, hashmapList);
						if (lev261 != null) {
							int k = 0;
							for (Mutator mut : lev261) {
								if (mut != null) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> levent62 = mutation2(packages, model, hashmapEObject, hashmapList);
						if (levent62 != null) {
							int k = 0;
							for (Mutator mut : levent62) {
								if (mut != null) {
									Object mutated = mut.mutate();
									if (mutated != null) {
										if (mutated instanceof EObject) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> l63 = mutation3(packages, model, hashmapEObject, hashmapList);
						if (l63 != null) {
							int k = 0;
							for (Mutator mut : l63) {
								if (mut != null) {
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/xee/data/out/",
					"C:/eclipse/workspace/xee/data/model/bpel.ecore", "C:/eclipse/workspace/xee/data/model/",
					"xee.java", hashmapMutVersions);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/xee/data/out/",
					"C:/eclipse/workspace/xee/data/model/bpel.ecore", "C:/eclipse/workspace/xee/data/model/",
					"xee.java", hashmapMutVersions);
			metricsGenerator.run();
			monitor.worked(1);
