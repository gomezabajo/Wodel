...
public class rst extends manager.MutatorUtils implements manager.IMutatorExecutor {
	...
	public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics,
		...
		for (String modelFilename : modelFilenames) {
			...
			Resource mutatormodel = ModelManager.loadModel(mutatorpackages,
					URI.createURI("file:/C:/eclipse/workspace/rst/data/out/rst.model").toFileString());
			...
			for (int i = 0; i < numMutants; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> l42 = mutation1(packages, model, hashmapEObject, hashmapList);
						if (l42 != null) {
							int k = 0;
							for (Mutator mut : l42) {
								if (mut != null) {
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/rst/data/out/",
					"C:/eclipse/workspace/rst/data/model/Asple.ecore", "C:/eclipse/workspace/rst/data/model/",
					"rst.java", hashmapMutVersions);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/rst/data/out/",
					"C:/eclipse/workspace/rst/data/model/Asple.ecore", "C:/eclipse/workspace/rst/data/model/",
					"rst.java", hashmapMutVersions);
			metricsGenerator.run();
			monitor.worked(1);
