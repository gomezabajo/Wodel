...
public class rli extends manager.MutatorUtils implements manager.IMutatorExecutor {
	...
	private ArrayList<Mutator> mutation2(List<EPackage> packages, Resource model, HashMap<String, EObject> hmObjects,
		...
		SpecificReferenceSelection referenceSelection = null;
		if (hmObjects.get("loop") != null) {
			referenceSelection = new SpecificReferenceSelection(packages, model, "expression", hmObjects.get("loop"));
		} else {
			return mutations;
		}
	...
	public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics,
		...
		String ecoreURI = "C:/eclipse/workspace/rli/data/model/Asple.ecore";
		String modelURI = "C:/eclipse/workspace/rli/data/model/";
		String modelsURI = "C:/eclipse/workspace/rli/data/out/";
		...
		for (String modelFilename : modelFilenames) {
			...
			Resource mutatormodel = ModelManager.loadModel(mutatorpackages,
					URI.createURI("file:/C:/eclipse/workspace/rli/data/out/rli.model").toFileString());
			...
			for (int i = 0; i < numMutants; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> lloop31 = mutation1(packages, model, hashmapEObject, hashmapList);
						if (lloop31 != null) {
							int k = 0;
							for (Mutator mut : lloop31) {
								if (mut != null) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> l32 = mutation2(packages, model, hashmapEObject, hashmapList);
						if (l32 != null) {
							int k = 0;
							for (Mutator mut : l32) {
								if (mut != null) {
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/rli/data/out/",
					"C:/eclipse/workspace/rli/data/model/Asple.ecore", "C:/eclipse/workspace/rli/data/model/",
					"rli.java", hashmapMutVersions);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/rli/data/out/",
					"C:/eclipse/workspace/rli/data/model/Asple.ecore", "C:/eclipse/workspace/rli/data/model/",
					"rli.java", hashmapMutVersions);
			metricsGenerator.run();
			monitor.worked(1);
