...
public class cis extends manager.MutatorUtils implements manager.IMutatorExecutor {
	...
	public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics,
		...
		for (String modelFilename : modelFilenames) {
			...
			Resource mutatormodel = ModelManager.loadModel(mutatorpackages,
					URI.createURI("file:/C:/eclipse/workspace/cis/data/out/cis.model").toFileString());
			...
			for (int i = 0; i < numMutants; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> ls019 = mutation1(packages, model, hashmapEObject, hashmapList);
						if (ls019 != null) {
							int k = 0;
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> ls120 = mutation2(packages, model, hashmapEObject, hashmapList);
						if (ls120 != null) {
							int k = 0;
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/cis/data/out/",
					"C:/eclipse/workspace/cis/data/model/PFSM.ecore", "C:/eclipse/workspace/cis/data/model/",
					"cis.java", hashmapMutVersions);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/cis/data/out/",
					"C:/eclipse/workspace/cis/data/model/PFSM.ecore", "C:/eclipse/workspace/cis/data/model/",
					"cis.java", hashmapMutVersions);
			metricsGenerator.run();
			monitor.worked(1);
