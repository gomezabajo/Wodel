...
public class mfs extends manager.MutatorUtils implements manager.IMutatorExecutor {
	...
	public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics,
		...
		for (String modelFilename : modelFilenames) {
			...
			Resource mutatormodel = ModelManager.loadModel(mutatorpackages,
					URI.createURI("file:/C:/eclipse/workspace/mfs/data/out/mfs.model").toFileString());
			...
			for (int i = 0; i < numMutants; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> l132 = mutation1(packages, model, hashmapEObject, hashmapList);
						if (l132 != null) {
							int k = 0;
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/mfs/data/out/",
					"C:/eclipse/workspace/mfs/data/model/DFAAutomaton.ecore", "C:/eclipse/workspace/mfs/data/model/",
					"mfs.java", hashmapMutVersions);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/mfs/data/out/",
					"C:/eclipse/workspace/mfs/data/model/DFAAutomaton.ecore", "C:/eclipse/workspace/mfs/data/model/",
					"mfs.java", hashmapMutVersions);
			metricsGenerator.run();
			monitor.worked(1);
