...
public class rtt extends manager.MutatorUtils implements manager.IMutatorExecutor {
	...
	private ArrayList<Mutator> mutation2(List<EPackage> packages, Resource model, HashMap<String, EObject> hmObjects,
		...
		((ReferenceEvaluation) exp0.first).value = new SpecificObjectSelection(packages, model, hmObjects.get("t1"))
				.getObject();
	...
	public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics,
		...
		for (String modelFilename : modelFilenames) {
			...
			Resource mutatormodel = ModelManager.loadModel(mutatorpackages,
					URI.createURI("file:/C:/eclipse/workspace/rtt/data/out/rtt.model").toFileString());
			...
			for (int i = 0; i < numMutants; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> lt181 = mutation1(packages, model, hashmapEObject, hashmapList);
						if (lt181 != null) {
							int k = 0;
							for (Mutator mut : lt181) {
								if (mut != null) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> lt282 = mutation2(packages, model, hashmapEObject, hashmapList);
						if (lt282 != null) {
							int k = 0;
							for (Mutator mut : lt282) {
								if (mut != null) {
									Object mutated = mut.mutate();
									if (mutated != null) {
										if (mutated instanceof EObject) {
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/rtt/data/out/",
					"C:/eclipse/workspace/rtt/data/model/SecurityPolicy.ecore", "C:/eclipse/workspace/rtt/data/model/",
					"rtt.java", hashmapMutVersions);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/rtt/data/out/",
					"C:/eclipse/workspace/rtt/data/model/SecurityPolicy.ecore", "C:/eclipse/workspace/rtt/data/model/",
					"rtt.java", hashmapMutVersions);
			metricsGenerator.run();
			monitor.worked(1);
