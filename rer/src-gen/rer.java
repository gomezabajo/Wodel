...
public class rer extends manager.MutatorUtils implements manager.IMutatorExecutor {
	...
	public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics,
		...
		for (String modelFilename : modelFilenames) {
			...
			Resource mutatormodel = ModelManager.loadModel(mutatorpackages,
					URI.createURI("file:/C:/eclipse/workspace/rer/data/out/rer.model").toFileString());
			...
			for (int i = 0; i < numMutants; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> l161 = mutation1(packages, model, hashmapEObject, hashmapList);
						if (l161 != null) {
							int k = 0;
							for (Mutator mut : l161) {
								if (mut != null) {
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/rer/data/out/",
					"C:/eclipse/workspace/rer/data/model/SecurityPolicy.ecore", "C:/eclipse/workspace/rer/data/model/",
					"rer.java", hashmapMutVersions);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/rer/data/out/",
					"C:/eclipse/workspace/rer/data/model/SecurityPolicy.ecore", "C:/eclipse/workspace/rer/data/model/",
					"rer.java", hashmapMutVersions);
			metricsGenerator.run();
			monitor.worked(1);
