...
public class uas2 extends manager.MutatorUtils implements manager.IMutatorExecutor {
	...
	private ArrayList<Mutator> mutation3(List<EPackage> packages, Resource model, HashMap<String, EObject> hmObjects,
		...
		atts.put("nsrc", new CopyAttributeConfigurationStrategy(
				(objectSelection != null ? objectSelection.getObject() : null), hmObjects.get("a1"), "nsrc", "nsrc"));
		atts.put("ntar", new CopyAttributeConfigurationStrategy(
				(objectSelection != null ? objectSelection.getObject() : null), hmObjects.get("a2"), "ntar", "ntar"));
		atts.put("derived", new SpecificBooleanConfigurationStrategy(true));
		...
		if (hmObjects.get("a1") != null) {
			refSelection31 = new SpecificObjectSelection(packages, model, hmObjects.get("a1"), "src");
		} else {
			if (hmList.get("a1") != null) {
				refSelection31 = new SpecificObjectSelection(packages, model, hmList.get("a1"), "src");
			} else {
				return mutations;
			}
		}
		...
		if (hmObjects.get("a2") != null) {
			refSelection32 = new SpecificObjectSelection(packages, model, hmObjects.get("a2"), "tar");
		} else {
			if (hmList.get("a2") != null) {
				refSelection32 = new SpecificObjectSelection(packages, model, hmList.get("a2"), "tar");
			} else {
				return mutations;
			}
		}
	...
	public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics,
		...
		String ecoreURI = "C:/eclipse/workspace/uas2/data/model/UMLClassDiagram.ecore";
		String modelURI = "C:/eclipse/workspace/uas2/data/model/";
		String modelsURI = "C:/eclipse/workspace/uas2/data/out/";
		...
		for (String modelFilename : modelFilenames) {
			...
			Resource mutatormodel = ModelManager.loadModel(mutatorpackages,
					URI.createURI("file:/C:/eclipse/workspace/uas2/data/out/uas2.model").toFileString());
			...
			for (int i = 0; i < numMutants; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> la1278 = mutation1(packages, model, hashmapEObject, hashmapList);
						if (la1278 != null) {
							int k = 0;
							for (Mutator mut : la1278) {
								if (mut != null) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> la2279 = mutation2(packages, model, hashmapEObject, hashmapList);
						if (la2279 != null) {
							int k = 0;
							for (Mutator mut : la2279) {
								if (mut != null) {
									Object mutated = mut.mutate();
									if (mutated != null) {
										if (mutated instanceof EObject) {
					...
					max = 1;
					for (int j = 0; j < max; j++) {
						ArrayList<Mutator> la3280 = mutation3(packages, model, hashmapEObject, hashmapList);
						if (la3280 != null) {
							int k = 0;
							for (Mutator mut : la3280) {
								if (mut != null) {
									Object mutated = mut.mutate();
									if (mutated != null) {
										if (mutated instanceof EObject) {
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/uas2/data/out/",
					"C:/eclipse/workspace/uas2/data/model/UMLClassDiagram.ecore",
					"C:/eclipse/workspace/uas2/data/model/", "uas2.java", hashmapMutVersions);
		...
		if (debugMetrics == true) {
			...
			metricsGenerator = new DebugMutatorMetricsGenerator(metricspackages, "C:/eclipse/workspace/uas2/data/out/",
					"C:/eclipse/workspace/uas2/data/model/UMLClassDiagram.ecore",
					"C:/eclipse/workspace/uas2/data/model/", "uas2.java", hashmapMutVersions);
			metricsGenerator.run();
			monitor.worked(1);
