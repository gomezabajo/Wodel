...
public class asi extends mutator.MutatorUtils implements manager.IMutatorExecutor {
...
//SELECT OBJECT mutation1
	...
	//REFERENCE: 
	((ReferenceEvaluation) exp0.first).name = "activities";
...
//SELECT OBJECT mutation2
	...
	if (hmObjects.get("s2") != null) {
		containerSelection = new SpecificObjectSelection(packages, model, hmObjects.get("s2"));
		   			} else {
		   				if (hmList.get("s2") != null) {
		   					containerSelection = new SpecificObjectSelection(packages, model, hmList.get("s2"));
		   				}
		   				else {
			return mutations;
		}
	...
	if (hmObjects.get("s2") != null) {
		referenceSelection = new SpecificReferenceSelection(packages, model, "activities", hmObjects.get("s2"));
			   			} else {
			   				return mutations;
	}
	RandomTypeSelection rts = new RandomTypeSelection(packages, model, "Activity", referenceSelection, containerSelection);
...
//METHOD NAME:mutation3
	...
	// REMOVE OBJECT mutation3
		...
		if (hmObjects.get("a1") != null) {
			objectSelection = new SpecificObjectSelection(packages, model, hmObjects.get("a1"), referenceSelection, containerSelection);
	   			} else {
			return mutations;
		}
...
//METHOD NAME:mutation4
	...
	ObSelectionStrategy objectSelection = null;
	if (hmObjects.get("s2") != null) {
		objectSelection = new SpecificObjectSelection(packages, model, hmObjects.get("s2"));
	} else {
		if (hmList.get("s2") != null) {
			objectSelection = new SpecificObjectSelection(packages, model, hmList.get("s2"));
		}
		else {
			return mutations;
		}
	...
	//NAME:activities
	   			if (objectSelection != null && objectSelection.getObject() != null) {
	   				if (hmObjects.get("a1") != null) {
	   					...
	   					if (refsList.get("activities") != null) {
				   				refs = refsList.get("activities");
		   					}
							else {
				   				refs = new ArrayList<ReferenceConfigurationStrategy>();
							}
		   					refs.add(new SpecificReferenceConfigurationStrategy(model, objectSelection.getObject(), hmObjects.get("a1"), "activities"));
		   					refsList.put("activities", refs);
			   			} else {
							return mutations;
						}
	   			}
...
public void execute(int maxAttempts, int numMutants, boolean registry, boolean metrics, boolean debugMetrics, IProgressMonitor monitor) throws ReferenceNonExistingException, WrongAttributeTypeException, 
	...
	HashMap<String, List<String>> hashmapMutVersions = new HashMap<String, List<String>>();
	    ...
	    String ecoreURI = "C:/eclipse/runtime-EclipseApplication/asi/data/model/bpel.ecore";
	    
	    String modelURI = "C:/eclipse/runtime-EclipseApplication/asi/data/model/";
	    String modelsURI = "C:/eclipse/runtime-EclipseApplication/asi/data/out/";
	    //String resourcesURI = "C:/eclipse/runtime-EclipseApplication/asi/resources/";
	    ...
	    for (String modelFilename : modelFilenames) {
	   	...
	   	Resource mutatormodel = ModelManager.loadModel(mutatorpackages, URI.createURI("file:/C:/eclipse/runtime-EclipseApplication/asi/data/out/asi.model").toFileString());
	   		...
	   		HashMap<String, List<EObject>> hashmapList = new HashMap<String, List<EObject>>();
	   			...
	   			Mutations muts = AppliedMutationsFactory.eINSTANCE.createMutations();
	   	   				...
	   	   				//RESET COUNTER: 0
	   	   	   			...
	   	   	   			ArrayList<Mutator> ls257 = mutation1(packages, model, hashmapEObject, hashmapList);
	   	   	   			//COUNTER: 1
	   	   	   			//REGISTRY METHOD NAME:registry1
	   	   	   			
	   	   	   			if (ls257 != null) {
	   	   	   				int k = 0;
	   	   	   				for (Mutator mut : ls257) {
	   	   	   					if (mut != null) {
	   	   	   						Object mutated = mut.mutate();
	   	   	   						if (mutated != null) {
	   	   	   							if (mutated instanceof EObject) {
	   	   	   							hashmapEObject.put("s2", mut.getObject());
	   	   	   							}
	   	   	   			...
	   	   	   			ArrayList<Mutator> la158 = mutation2(packages, model, hashmapEObject, hashmapList);
	   	   	   			//COUNTER: 2
	   	   	   			//REGISTRY METHOD NAME:registry2
	   	   	   			
	   	   	   			if (la158 != null) {
	   	   	   				int k = 0;
	   	   	   				for (Mutator mut : la158) {
	   	   	   					if (mut != null) {
	   	   	   						Object mutated = mut.mutate();
	   	   	   						if (mutated != null) {
	   	   	   							if (mutated instanceof EObject) {
	   	   	   							hashmapEObject.put("a1", mut.getObject());
	   	   	   							}
	   	   	   			...
	   	   	   			ArrayList<Mutator> l59 = mutation3(packages, model, hashmapEObject, hashmapList);
	   	   	   			//COUNTER: 3
	   	   	   			...
	   	   	   			ArrayList<Mutator> l60 = mutation4(packages, model, hashmapEObject, hashmapList);
	   	   	   			//COUNTER: 4
	...
	Bundle bundle = Platform.getBundle("wodel.models");
   	...
   	String metricsecore = FileLocator.resolve(fileURL).getFile();
   		//String metricsecore = "C:/eclipse/runtime-EclipseApplication/asi/resources/MutatorMetrics.ecore";
   		
   		if (metrics == true) {
   			ArrayList<EPackage> metricspackages = ModelManager.loadMetaModel(metricsecore);
   			monitor.subTask("Generating dynamic net metrics");
   			MutatorMetrics.generateMetrics(metricspackages, "C:/eclipse/runtime-EclipseApplication/asi/data/out/", "C:/eclipse/runtime-EclipseApplication/asi/data/model/bpel.ecore", "C:/eclipse/runtime-EclipseApplication/asi/data/model/", "asi", "C:/eclipse/runtime-EclipseApplication/asi", hashmapMutVersions);
   			monitor.worked(1);
   		}
   		if (debugMetrics == true) {
		ArrayList<EPackage> metricspackages = ModelManager.loadMetaModel(metricsecore);
   			monitor.subTask("Generating dynamic debug metrics");
   			MutatorMetrics.generateDebugMetrics(metricspackages, "C:/eclipse/runtime-EclipseApplication/asi/data/out/", "C:/eclipse/runtime-EclipseApplication/asi/data/model/bpel.ecore", "C:/eclipse/runtime-EclipseApplication/asi/data/model/", "asi", "C:/eclipse/runtime-EclipseApplication/asi", hashmapMutVersions);
   			monitor.worked(1);   			
   		}
   	}
}
