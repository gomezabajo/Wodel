package mutator.designerDynamic;
...
public class designerDynamic extends MutatorUtils {
	...
	private List<Mutator> mutation1(List<EPackage> packages, Resource model,
		...
		List<EObject> containers = ModelManager.getParentObjects(packages, model, "Automaton");
	...
	public MutationResults execute(int maxAttempts, int numMutants, boolean registry, boolean metrics,
		...
		String modelURI = designerDynamic.class.getProtectionDomain().getCodeSource().getLocation().getPath()
				.replace("/bin/", "/") + "data/model/";
		String modelsURI = designerDynamic.class.getProtectionDomain().getCodeSource().getLocation().getPath()
		...
		for (String modelFilename : modelFilenames) {
			...
			String xmiFilename = designerDynamic.class.getProtectionDomain().getCodeSource().getLocation().getPath()
			...
			for (int i = 0; i < numMutantsToGenerate; i++) {
				...
				while ((isRepeated == true) && (attempts < maxAttempts)) {
					...
					mutantIndex[0] = i;
					isRepeated = registryMutant(ecoreURI, packages, registeredPackages, localRegisteredPackages, seed,
							model, rules, muts, modelFilename, mutFilename, registry, hashsetMutants,
							hashmapModelFilenames, mutantIndex, mutPaths, hashmapMutVersions, project, serialize, test,
							classes, this.getClass(), true);
					if (isRepeated == false) {
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(metricspackages,
					designerDynamic.class.getProtectionDomain().getCodeSource().getLocation().getPath().replace("/bin/",
							"/") + "data/out/",
					"/designer/data/model/DFAAutomaton.ecore", designerDynamic.class.getProtectionDomain()
